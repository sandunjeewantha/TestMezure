...
<body>
  ...
  <nav class="navbar bg-body-tertiary">
    <div class="container-fluid">
      <form class="d-flex" method="GET" action="posts.php">
        <input class="form-control me-2" type="search" placeholder="Search" name="fl_search" aria-label="Search">
        <button class="btn btn-outline-success" type="submit">Search</button>
        <a href="posts.php" class="btn btn-outline-success" style="margin-left: 10px;">Clear</a>
      </form>
    </div>
  </nav>
  <br>
  ...
  while ($rowqu = mysqli_fetch_assoc($quections)) {
    $valu = strtolower($rowqu['quection']);
    if (!strpos($valu, $fl_search) !== false && $fl_search != "") {
      continue;
    }
